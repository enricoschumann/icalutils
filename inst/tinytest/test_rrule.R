...
res1 <- rrule(dtstart = as.POSIXct("1997-09-02 09:00:00"),
             freq = "daily",
             count = 10)
res2 <- rrule(dtstart = as.POSIXct("1997-09-02 09:00:00"),
             text = "RRULE:FREQ=DAILY;COUNT=10")
expect_equal(res1$recurrence_set, res2$recurrence_set)
...
res <- rrule(dtstart = as.POSIXct("1997-09-02 09:00:00"),
             text = "FREQ=DAILY;UNTIL=19971224T000000Z")
expect_equal(as.Date(res$recurrence_set$DTSTART),
             seq(as.Date("1997-09-02"), as.Date("1997-12-23"), by = "1 day"))
...
##                               December 1,3,...
res <- rrule(dtstart = as.POSIXct("1997-09-02 09:00:00"),
             text = "RRULE:FREQ=DAILY;INTERVAL=2")
expect_true(all(as.POSIXlt(res$recurrence_set$DTSTART)$hour == 9L))
expect_true(all(diff(as.Date(res$recurrence_set$DTSTART)) == 2))
...
res <- rrule(dtstart = as.POSIXct("1997-09-02 09:00:00"),
             text = "FREQ=DAILY;INTERVAL=10;COUNT=5")
expect_true(all(as.POSIXlt(res$recurrence_set$DTSTART)$hour == 9L))
expect_true(all(diff(as.Date(res$recurrence_set$DTSTART)) == 10))
expect_equal(length(res$recurrence_set$DTSTART), 5)
...
res <- rrule(dtstart = as.POSIXct("2007-01-15 09:00:00"),
             text = "RRULE:FREQ=MONTHLY;BYMONTHDAY=15,30;COUNT=5")
expect_true(all(as.POSIXlt(res$recurrence_set$DTSTART)$hour == 9L))
expect_equal(length(res$recurrence_set$DTSTART), 5)
expect_equal(as.Date(res$recurrence_set$DTSTART),
             as.Date(c("2007-01-15",
                       "2007-01-30",
                       "2007-02-15",
                       "2007-03-15",
                       "2007-03-30")))
